...
const AssetData = ({ id, title, end }: AssetDataProps) => {
    const { addAsset, deleteAsset } = useAssetStore((state) => state.actions);
    console.log('AssetData rendered')
    return (
        <article>
            <h2>{title}</h2>
            <AssetGroup id={id} />
            <section className='flex justify-end mt-[30px]'>
                <div className="flex gap-5 items-center">
                    <button className="label-1-normal-regular text-red-500 py-2 px-[10px] text-nowrap"
