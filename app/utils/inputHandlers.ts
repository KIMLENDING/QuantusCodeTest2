...
export const handleWeightChange = (value: string, onChange?: (v: string) => void) => {
    ...
    if (value.length > 4) return onChange?.('');
...
/** 숫자만 허용 핸들러 */
export const handleNumber = (value: string, onChange?: (v: string) => void) => {
    // 숫자만 허용
    if (!/^\d*$/.test(value)) return;
    if (value.length > 2) return onChange?.(''); // 최대 2자리 숫자만 허용
    onChange?.(value);
};

/** 5부터 200까지 입력 가능 핸들러 */
export const handlePeriodChange = (value: string, onChange?: (v: string) => void) => {
    if (!/^\d*$/.test(value)) return;
    const numericValue = parseInt(value, 10);
